...
class BinarySearchTreeTest(unittest.TestCase):
    ...
    # Test can sort data
    def test_6can_sort_single_number(self):
        self.assertEqual(BinarySearchTree(["2"]).sorted_data(), ["2"])

    def test_7can_sort_if_second_number_is_smaller_than_first(self):
        self.assertEqual(BinarySearchTree(["2", "1"]).sorted_data(), ["1", "2"])

    def test_8can_sort_if_second_number_is_same_as_first(self):
        self.assertEqual(BinarySearchTree(["2", "2"]).sorted_data(), ["2", "2"])

    def test_9can_sort_if_second_number_is_greater_than_first(self):
        self.assertEqual(BinarySearchTree(["2", "3"]).sorted_data(), ["2", "3"])

    def test_Acan_sort_complex_tree(self):
        self.assertEqual(
